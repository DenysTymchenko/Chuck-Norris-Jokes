...
export function showFavorite() {
  ...
  if (window.innerWidth <= 1090 && window.innerWidth > 655) {
    favoriteSection.classList.add('show-tablet');

    const blur = document.createElement('div');
    blur.classList.add('blur');
    blur.addEventListener('click', closeFavorite);

    mainSection.append(blur);
  } else {
    mainSection.style.display = 'none';
    favoriteSection.classList.add('show-mobile');
  }
}

export function closeFavorite() {
  const mainSection = document.querySelector('.main');
  const favoriteSection = document.querySelector('section.favorite');
  ...
  if (window.innerWidth <= 1090 && window.innerWidth > 655) {
    ...
    favoriteSection.classList.remove('show-mobile');
  }
}